...
class DownloaderUserbot:
    ...
    async def process_facebook_url(self, url: str, max_wait_time: int = 60) -> Optional[int]:
        ...
        try:
            ...
            # Send Facebook URL to @FacebookAsBot
            ...
            # Wait for response with video (Facebook bot sends MP4 files directly without inline buttons)
            video_message = await self._wait_for_video_response(
            ...
            if not video_message:
                logger.error(f"No video response received from @{self.facebook_bot_username}")
                return None
    ...
    async def process_twitter_url(self, url: str, max_wait_time: int = 60) -> Optional[int]:
        """
        ...
        try:
            # Send Twitter URL to @twittervid_bot
            ...
            # Wait for response with media (video or photo) and caption
            media_message = await self._wait_for_twitter_response(
            ...
            storage_message_id = await self._upload_twitter_to_storage_channel(media_message)
    ...
    async def _wait_for_twitter_response(self, bot_username: str, sent_message_id: int, max_wait_time: int) -> Optional[Message]:
        """
        Wait for media response from Twitter bot.
        ...
        Returns:
            Message with media or None if failed
        """
        start_time = asyncio.get_event_loop().time()
        
        while (asyncio.get_event_loop().time() - start_time) < max_wait_time:
            try:
                # Check the last 10 messages to account for multiple responses
                ...
                for message in messages:
                    if message.id > sent_message_id:
                        ...
                        if is_video or is_photo:
                            ...
                            logger.info(f"Found Twitter {media_type} message ID: {message.id}")
                            return message
    ...
    async def _wait_for_format_message(self, bot_username: str, sent_message_id: int, max_wait_time: int) -> Optional[Message]:
        """
        Wait for format selection message from @SaveYoutubeBot.
        ...
        while (asyncio.get_event_loop().time() - start_time) < max_wait_time:
            try:
                ...
                for message in messages:
                    # Check if this is a response with inline keyboard
                    if (message.id > sent_message_id and 
                        message.reply_markup and 
                        hasattr(message.reply_markup, 'rows')):
                        
                        logger.info(f"Received format selection message from @{bot_username}")
                        return message
